...
editCandidate(c: any) {
  ...
  this.recruitmentService.getCandidateById(c.candidateId).subscribe(res => {
...
loadCandidateDetails(candidateId: number) {
  this.recruitmentService.getCandidateById(candidateId).subscribe(res => {
    this.experienceList = res.experiences.map((e: any) => ({
      from: e.fromYear,
      to: e.toYear,
      designation: e.designation,
