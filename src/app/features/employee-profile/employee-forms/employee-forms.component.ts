...
viewDocument(fileName: string) {
  ...
  // If the fileName already has folder path, don't add it again
  let fileUrl = fileName.includes("Uploads/EmployeeForms")
    ? `${environment.apiUrl}/${fileName}`
    : `${environment.apiUrl}/Uploads/EmployeeForms/${fileName}`;
